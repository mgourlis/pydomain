...
Structure each description — classify the task by type and apply the corresponding template from [`${CLAUDE_SKILL_DIR}/../task-creation/references/issue-patterns.md`]:

- **Bug** → use Bug Report template (Context, Problem, Steps to Reproduce, Expected/Actual, AC)
- **Investigation** → use Investigation template (Context, Questions, Goal, Constraints, AC)
- **Feature/Story** → use Feature or Story template (Context, What to Do, AC, Out of Scope)
- **Documentation** → use Documentation template (Context, What to Document, Location, Audience, AC)
- **Default (Task)** → use Implementation template (Context, What to Do, AC, References)
...
- **[`${CLAUDE_SKILL_DIR}/../task-creation/references/issue-patterns.md`]** — Issue type taxonomy and per-type description templates for classifying decomposed tasks and writing type-appropriate descriptions. Quality guardrails for catching vague drafts before presentation.
