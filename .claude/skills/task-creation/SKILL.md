...
**Priority-tagged (Should/Must/Can)** — Best for features with mixed priority requirements. Distinguishes hard requirements from nice-to-haves:
...
Use rule-oriented by default. Switch to scenario-oriented when the behavior depends on specific preconditions or multi-step sequences. Use priority-tagged when the task has a mix of hard and soft requirements.

For the full AC coverage checklist (6 categories) and minimum AC counts by complexity, see [`${CLAUDE_SKILL_DIR}/references/issue-patterns.md`].
...
For the full issue type taxonomy (Epic, Story, Feature, Documentation, Prompt-style) and per-type templates, see [`${CLAUDE_SKILL_DIR}/references/issue-patterns.md`]. The templates below cover the four most common patterns.
...
- **[`${CLAUDE_SKILL_DIR}/references/issue-patterns.md`]** — Issue type taxonomy (Epic, Story, Task, Bug, Feature, Investigation, Documentation, Prompt-style), per-type description templates with story subtypes, section header conventions, AC patterns (Checklist, Given-When-Then, Should/Must/Can), AC coverage checklist and minimum AC counts, INVEST validation with failure patterns, quality guardrails including antipatterns, and clarification thresholds
...
- [ ] Title is imperative, specific, and 5-10 words — no vague titles like "Fix bug" or "Improve X"
- [ ] Description includes context linking to the larger effort
- [ ] Work items are concrete but don't prescribe implementation
- [ ] Acceptance criteria are testable with clear pass/fail outcomes — no subjective criteria like "works correctly"
- [ ] Every criterion has a measurable threshold or clear boolean outcome
- [ ] Negative cases covered — error conditions, empty states, invalid input where relevant
- [ ] Out of Scope section present when the task could be interpreted broadly
