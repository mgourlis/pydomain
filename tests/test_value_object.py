...
class TestFrozenImmutability:
    ...
    def test_cannot_set_attribute_via_model_config(self) -> None:
        c = Color(name="red", hex_code="#FF0000")
        with pytest.raises(ValidationError):
...
class TestValueEquality:
    ...
    def test_not_equal_when_any_field_differs(self) -> None:
        a = Color(name="red", hex_code="#FF0000")
        b = Color(name="blue", hex_code="#0000FF")
        assert a != b

    def test_equal_with_different_types_not_equal(self) -> None:
        c = Color(name="red", hex_code="#FF0000")
        assert c != "red"
...
class TestHashability:
    ...
    def test_not_equal_objects_have_different_hashes(self) -> None:
        a = Color(name="red", hex_code="#FF0000")
        b = Color(name="blue", hex_code="#0000FF")
...
class TestSerialization:
    ...
    def test_model_dump_with_decimal_round_trip(self) -> None:
        original = Money(amount=Decimal("10.50"), currency="USD")
        data = original.model_dump()
        restored = Money.model_validate(data)
        assert restored == original

    def test_model_dump_json_with_decimal_round_trip(self) -> None:
        original = Money(amount=Decimal("10.50"), currency="USD")
        json_str = original.model_dump_json()
...
class TestClosureOfOperations:
    def test_model_copy_creates_new_instance_with_updates(self) -> None:
    ...
    def test_subclass_add_returns_new_instance(self) -> None:
    ...
    def test_mul_returns_new_instance(self) -> None:
        m = Money(amount=Decimal("10.00"), currency="USD")
        result = m * 3
        assert result.amount == Decimal("30.00")
        assert result.currency == "USD"

    def test_add_with_different_currency_raises(self) -> None:
        a = Money(amount=Decimal("10.00"), currency="USD")
...
class TestValueObjectInheritance:
    def test_subclass_preserves_frozen(self) -> None:
    ...
    def test_subclass_with_validators(self) -> None:
